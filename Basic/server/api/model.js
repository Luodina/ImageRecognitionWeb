...
router.post('/:modelName', function (req, res) {
  ...
  let userName = req.user.username; //req.body.USER_ID;
...
router.post('/publish/:modelName', function (req, res) {
  let modelName = req.params.modelName;
  let status = '已发布';
  let publishModelName = req.body.publishModelName;
  // console.log('----qqqqq', modelName, publishModelName);
  Model.update(
    {PUBLISH_MODEL_NAME: publishModelName, STATUS: status},
    {where: {APP_ID: modelName}}).then(() => {
    res.send({msg: 'success'});
  })
    .catch(err => {
      console.log('err', err);
